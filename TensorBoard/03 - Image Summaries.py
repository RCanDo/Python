...
version: 1.0
type: code chunks
keywords: [TensorBoard, Keras, image summaries]
description: Examples.
sources:
    - title: Displaying image data in TensorBoard
      link: https://www.tensorflow.org/tensorboard/image_summaries
      usage: copy
file:
    usage: 
        interactive: True
        terminal: False
    name: "03 - Image Summaries.py"
...
Using the TensorFlow Image Summary API, you can easily log tensors 
and arbitrary images and view them in TensorBoard. 
This can be extremely helpful to sample and examine your input data, 
or to visualize layer weights and generated tensors. 
You can also log diagnostic data as images that can be helpful 
in the course of your model development.

In this tutorial, you will use learn how to use the Image Summary API 
to visualize tensors as images. 
You will also learn how to take an arbitrary image, convert it to a tensor, 
and visualize it in TensorBoard. 
You will work through a simple but real example that uses Image Summaries 
to help you understand how your model is performing.
"""
#%% Setup
"""
...
"""
You're going to construct a simple neural network to classify images 
in the the Fashion-MNIST dataset. 
...
"""
print("Shape: ", train_images[0].shape)  # Shape:  (28, 28)
print("Label: ", train_labels[0], "->", class_names[train_labels[0]]) # Label:  9 -> Ankle boot
...
However, tf.summary.image() expects a rank-4 tensor containing 
 (batch_size, height, width, channels). 
Therefore, the tensors need to be reshaped.

You're logging only one image, so batch_size is 1. 
The images are grayscale, so set channels to 1.
"""
...
with file_writer.as_default():
  tf.summary.image("Training data", img, step=0)   # step=0 ???

"""
Now, use TensorBoard to examine the image. Wait a few seconds for the UI to spin up.
%tensorboard --logdir logs/train_data --host 127.0.0.1
(it's better to run it from some terminal like anaconda prompt)
...
In the code below, you'll log the first 25 images as a nice grid 
...
You're going to use image summaries to understand how well your model is doing 
while training a simple classifier for the Fashion-MNIST dataset.
...
#%%
"""
When training a classifier, it's useful to see the confusion matrix. 
...
"""# Start TensorBoard.
%tensorboard --logdir logs\\image  --host 127.0.0.1
"""
...
with darker squares coalescing along the diagonal, 
and the rest of the matrix tending toward 0 and white. 
...
If you're interested, try to improve this model with a convolutional network 
(CNN)[https://medium.com/tensorflow/hello-deep-learning-fashion-mnist-with-keras-50fcff8cd74a].
"""
