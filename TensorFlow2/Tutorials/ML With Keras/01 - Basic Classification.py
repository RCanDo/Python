...
file:
    usage: 
        interactive: True
        terminal: False
    name: "01 - Basic Classification.py"
...
This guide uses the [Fashion MNIST](https://github.com/zalandoresearch/fashion-mnist) 
...
This guide uses Fashion MNIST for variety, and because it's a slightly more challenging problem 
than regular MNIST. 
...
to build and train the network, let's display the first 25 images 
...
After the pixels are flattened, the network consists of 
a sequence of two tf.keras.layers.Dense layers. 
These are densely connected, or fully connected, neural layers. 
The first Dense layer has 128 nodes (or neurons). 
The second (and last) layer is a 10-node softmax layer 
that returns an array of 10 probability scores that sum to 1. 
...
These are added during the model's compile step:
    ...
    Metrics — Used to monitor the training and testing steps. 
        The following example uses accuracy,
...
than the accuracy on the training dataset. 
...
They represent the model's "confidence" that the image corresponds to each 
