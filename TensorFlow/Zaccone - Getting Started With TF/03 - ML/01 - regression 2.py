...
description: The same example as in "01 - regression.py" but we moved xx and yy to tf.canstant().
...
yy = a * xx + b + np.random.normal(0, sigma, N)
...
xx = tf.constant(xx, dtype="float32")
yy = tf.constant(yy, dtype="float32")

# initial values of parameters
a_hat = tf.Variable(tf.random_uniform([1], -1., 1.))
b_hat = tf.Variable(tf.zeros([1]))
yy_hat = tf.add(tf.multiply(a_hat, xx), b_hat)
...
with tf.Session() as ss:
    ...
    for k in range(21):
        ss.run(train)
        if k % 5 == 0:
            plt.plot(xx.eval(), yy.eval(), '.', label = 'step {}'.format(k))
            yy_hat = ss.run(a_hat) * xx.eval() + ss.run(b_hat)
            plt.plot(xx.eval(), yy_hat)
            plt.legend()
            plt.show()
