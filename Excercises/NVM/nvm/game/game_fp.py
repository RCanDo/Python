...
class Game(object):
    ...
    def play(self, actions):
        ...
        try:
            reduce(lambda ok, a: ACT[a](), list(actions), True)
        except Exception:
            raise ValueError("Expected string of 'M', 'L' or 'R' e.g. 'MMRMLML'.")
        # unfortunatelly: ROLLBACK does not work this way
        # although reduce() performs it in some cases but apparently not when using .methods() :(
