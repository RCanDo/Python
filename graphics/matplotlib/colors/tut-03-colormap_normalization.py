...
version: 1.0
type: tutorial
keywords: [colormap, normalization]
description: |
remarks:
    -
todo:
...
file:
    ...
    name: tut-03-colormap_normalization.py
...
"""
Objects that use colormaps by default linearly map the colors in the colormap
from data values `vmin` to `vmax`.
"""
pcm = ax.pcolormesh(x, y, Z, vmin=-1., vmax=1., cmap='RdBu_r')
"""
will map the data in Z linearly from -1 to +1,
so Z=0 will give a color at the center of the colormap RdBu_r (white in this case).

Matplotlib does this mapping in two steps,
with a normalization from the input data to [0, 1] occurring first,
and then mapping onto the indices in the colormap.

Normalizations are classes defined in the matplotlib.colors() module.
The default, linear normalization is matplotlib.colors.Normalize().

Artists (e.g. `ax.pcolormesh()`) that map data to color pass the arguments `vmin` and `vmax`
to construct a matplotlib.colors.Normalize() instance, then call it:
"""
norm = mpl.colors.Normalize(vmin=-1, vmax=1)
norm    # <matplotlib.colors.Normalize at 0x23aa7eb5580>
norm(0)
# 0.5
dir(norm)
vars(norm)
...
#%% Logarithmic
"""
One of the most common transformations is to plot data by taking its logarithm (to the base-10).
This transformation is useful to display changes across disparate scales.
Using `colors.LogNorm` normalizes the data via log10.

In the example below, there are two bumps, one much smaller than the other.
Using colors.LogNorm, the shape and location of each bump can clearly be seen:
"""
...
fig.colorbar(pcm, ax=ax[1], extend='max')    # extend -- see the arrow
plt.show()

#%% Centered
"""
In many cases, data is symmetrical around a center,
for example, positive and negative anomalies around a center 0.
In this case, we would like the center to be mapped to 0.5
and the datapoint with the largest deviation from the center to be mapped to 1.0,
if its value is greater than the center, or 0.0 otherwise.

The norm `colors.CenteredNorm` creates such a mapping automatically.
It is well suited to be combined with a divergent colormap
which uses different colors edges that meet in the center at an unsaturated color.
...
# pc = ax2.pcolormesh(Z, norm=colors.CenteredNorm(), cmap=cmap)   # mpl 3.4.3
pc = ax2.pcolormesh(Z, norm=colors.DivergingNorm(Z.mean()), cmap=cmap)   # mpl 3.3.4
...
"""
Similarly, it sometimes happens that there is data that is positive and negative,
but we would still like a logarithmic scaling applied to both.
In this case, the negative numbers are also scaled logarithmically, and mapped to smaller numbers; e.g., if vmin=-vmax, then the negative numbers are mapped from 0 to 0.5 and the positive from 0.5 to 1.
...
Sometimes it is useful to remap the colors onto a power-law relationship (i.e. y=xγ
, where γ

is the power). For this we use the colors.PowerNorm. It takes as an argument gamma (gamma == 1.0 will just yield the default linear normalization):
