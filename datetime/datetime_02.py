...
title: More on datetime lib.
subtitle:
version: 1.0
type: tutorial
keywords: [datetime, date, time]
description: |
    About dates and time in datetime library.
sources:
...
print(now)
dir(now)
now.tzinfo      # time-zone unaware
# but only formally; in fact it works as if time zone was set to local
# and interpreter knows the local TZ and DST from OS – see  timezone_01.py
...
# one must do this to get current time only
...
import time
d1.timetuple()
# time.struct_time(tm_year=2021, tm_mon=4, tm_mday=12,
#                   tm_hour=0, tm_min=0, tm_sec=0, tm_wday=0, tm_yday=102, tm_isdst=-1)
# ! this is basic date-time structure of  time  library
time.struct_time((2021, 4, 12, 0, 0, 0, 0, 102, -1))
# time.struct_time(tm_year=2021, tm_mon=4, tm_mday=12,
#                   tm_hour=0, tm_min=0, tm_sec=0, tm_wday=0, tm_yday=102, tm_isdst=-1)

# ...  see  timezone_01.py,  time.py (not exists yet)
...
dt1.timestamp()   #! 1618241415.123456  number of seconds since epoch = 1970-01-01 00:00:00
dt1.fromtimestamp(1618241415.123456)    # datetime.datetime(2021, 4, 12, 17, 30, 15, 123456)
dt.datetime.fromtimestamp(1618241415.123456)  # "
